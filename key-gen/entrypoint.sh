...
openssl x509 -req -days 365 -in $CERT_PATH/$CERT_NAME.csr -CA $CERT_PATH/IntermediateCA.crt -CAkey $CERT_PATH/IntermediateCA.key -CAcreateserial -out $CERT_PATH/$CERT_NAME.crt -sha256 -extfile <(echo subjectAltName=$SUBJECT_ALT_NAME)

rm $CERT_PATH/IntermediateCA.crt
